...
osc(x, t, o)=(*p; s=t?0:p; p=(s+pi2*x/sr)%pi2; s+o);
tri(x, t, o)=1-abs(1-(((osc(x, t, o)+hpi)/pi)%2))*2;
saw(x, t, o)=1-(((osc(x, t, o)+pi)/pi)%2);
ramp(x, t, o)=(((osc(x, t, o)+pi)/pi)%2)-1;
sqr(x, t, o)=ramp(x, t, o)<0?-1:1;
...
#noise=[..1s | rand];
#sine=[..1s | sine];
#exp=[..10s | x ->exp(-inc(1)/1s)];

wt(table, hz)=(*p; s=table[p]; p=(p+hz)%sr; s);
...
foldback(x, e)=(((x < -e) || (x > e)) ? (abs(abs((x - e) % (4 * e)) - 2 * e) - e) * (1 / e) : x);
...
freeverb(x0,
  ro ~ 0.01..0.999 = 0.5,
  da ~ 0.01..0.999 = 0.5)=(#a0=[..0.02530612237751484s];
  ...
  o=comb(x0, #a0, fb, da) +comb(x0, #a1, fb, da) +comb(x0, #a2, fb, da) +comb(x0, #a3, fb, da) +comb(x0, #b0, fb, da) +comb(x0, #b1, fb, da) +comb(x0, #b2, fb, da) +comb(x0, #b3, fb, da);
...
lop(x, freq ~ 1..nyq=1000.0)=(w=clamp05((tau * freq) / sr);
  b1=exp(-tau * w);
  a0=1f - b1;
  x*a0+x*b1);

hop(x, freq[1..nyq]=1000f)=(w=clamp05((tau * freq) / sr);
  b1=-exp(-tau * (0.5 - w));
  a0=1f + b1;
  x*a0+x*b1);

bi_co(freq[1..10k]=1000f,
  Q[0.01..2f]=1.0)=(w=(pi2 * freq) / sr;
  sin_w=sin(w);
  cos_w=cos(w);
  a=sin_w / (2.0 * Q);
  (sin_w, cos_w, a));

bi_ig(x0, b0, b1, b2, a0, a1, a2)=(*(x1, x2, y1, y2);
...
lp(x0, freq ~ 1..10k=1000.0, Q ~ 0.01..2.0=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
hp(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
bp(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
pk(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
notch(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
ap(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
pks(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0, gain[0.01..10f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
ls(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0, gain[0.01..10f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
hs(x0, freq[1..1000]=100.0, Q[0.01..2f]=1.0, gain[0.01..10f]=1.0)=((sin_w, cos_w, a)=bi_co(freq, Q);
...
svf_co(cutoff[50..22k]=3169.975,
  res[0.01..0.985]=0.556)=(g=tan(pi*cutoff/sr);
  k=2f-2f*res;
  a1=1f/(1f+g*(g+k));
  a2=g*a1;
  a3=g*a2;
...
svf(v0,
  ...
  res[0.01..0.985]=0.556)=((k, a1, a2, a3)=svf_co(cutoff, res);
  (v1, v2, v3)=svf_ig(v0, a1, a2, a3);
  (k, v1, v2, v3));

lpf(v0,
  cutoff[50..22k]=3169.975,
  res[0.01..0.985]=0.556)=((k, v1, v2, v3)=svf(v0, cutoff, res);
...
mooglp(x0,
  cut[0.1f..5k]=629.345,
  q[.1..4f]=0.706)=((x1, az3, az4, m_amf, m_postGain)=moog_ig(x0, cut, q);
  m_amf * m_postGain);
...
spline(#, i)=(fr=fract(i);
  ...
  x0 + 0.04166666666 * fr * ((x1-xm1) * 16.0+(xm2-x2) * 2.0 + fr * ((x1+xm1) * 16.0-xm2 -x0 * 30.0 - x2 + fr * (x1*66.0 - x0 * 70.0-x2 * 33.0+xm1 * 39.0 + x3*7.0 - xm2 * 9.0 + fr * (x0*126.0 - x1 * 124.0+x2 * 61.0-xm1 * 64.0 - x3*12.0 + xm2 * 13.0 + fr * ((x1 - x0) * 50.0+(xm1-x2) * 25.0 +(x3 - xm2) * 5.0))))));
...
daverb(x,
  ...
  lo=#d9(0.008937872s) + #d9(0.099929438s) - #d10(0.064278754s) + #d11(0.067067639s) - #d5(0.066866033s) - #d6(0.006283391s) - #d7(0.035818689s);

  ro=#d5(0.011861161s) + #d5(0.121870905s) - #d6(0.041262054s) + #d7(0.08981553s) - #d9(0.070931756s) - #d10(0.011256342s) - #d11(0.004065724s);
...
tb303(x,
  ...
  sat[0.01f..1.5f]=0.189,
  pre[0.1f..2f]=0.624,
  dist[1f..80f]=80)=atan(diode(x*pre, freq, Q, hpf, sat)*dist);

distort(#x, #y, #k, bias=0f, post=0f, x)=(
  xmin ~= ..#x[..]; \ xmin ~= ..#x[0], xmin ~= ..#x[1]
  xmax ~= #x[..]..;

  x+=bias;

  x ~= (xmin-0.001)..(xmax+0.001);
  ...
  q=(1-z) * #y(i-1) +z * #y(i) +z * (1-z) *(a * (1-z) + b * z);

  q+post);

#jfetl_x=[ -2.4999032020568848,
-1.7674853801727295,
-1.6253507137298584,
-1.5339927673339844,
-1.4729481935501099,
...
0,
0];
distort_jfetl(x)=distort(#jfetl_x,
  #jfetl_y,
  #jfetl_k,
  0, -5, x);
` \ export const dly = `
