...
var build = {
    ...
    const consume = (nodes, out = []) => {
      ...
      if (opCode >= 268) {
        ...
        if (opCode <= 11) {
          ...
          for (let i = 0; i < 16; i++)
            immed.push(i32.parse(args.shift()));
        } else if (opCode === 12) {
          args.unshift(op2);
  ...
  global([, ...args], ctx) {
    ...
    ctx.global.push([TYPE[mut ? type[1] : type], mut, ...consumeConst(init, ctx), 11]);
  },
  // (table 1 2? funcref)
...
var consumeConst = (node, ctx) => {
  let op2 = node.shift(), [type, cmd] = op2.split(".");
  if (type === "global")
    return [35, ...uleb(node[0][0] === "$" ? ctx.global[node[0]] : node[0])];
  if (type === "v128")
    return [253, 12, ...v128(node)];
  if (cmd === "const")
    return [65 + ["i32", "i64", "f32", "f64"].indexOf(type), ...encode_exports[type](node[0])];
  return [
...
var consumeParams = (args) => {
  ...
  while (args[0]?.includes("="))
    param = args.shift().split("="), params[param[0]] = Number(param[1]);
  return params;
};
...
function get(name) {
  ...
  if (locals[name].static)
    return op(`(global.get $${locals[name].static})`, locals[name].type);
...
function tee(name, init = "") {
  ...
  if (locals[name].static)
    return op(`(global.set $${locals[name].static} ${init})(global.get $${locals[name].static})`, locals[name].type);
...
function compile(node, config = {}) {
  ...
  for (let id2 of ids(lastNode))
    if (!id2.includes(":"))
      exports[id2] = globals[id2] || funcs[id2] || err2("Unknown export member `" + id2 + `'`);
...
`;
  ...
  for (let name in globals)
    if (!globals[name].import)
      code += `(global $${name} (mut ${globals[name].type}) ${globals[name].init || `(${globals[name].type}.const 0)`})
`;
  code += `
`;
  ...
  "[]"([, [, ...inits]], out) {
    ...
    for (let init of inits) {
      if (init[0] === "..") {
        ...
        str3 += f642.store(i322.sub(tee(ptr, i322.add(get(ptr), i322.const(8))), i322.const(8)), float(expr3(init)));
    }
    include("malloc"), include("arr.ref");
    str3 += `(global.set $__mem ${get(ptr)})`;
    depth--;
    if (out)
      return op(str3 + call("arr.ref", `${get(start)} (i32.shr_u (i32.sub ${get(ptr)} ${get(start)}) ${i322.const(3)})
`), "f64", { buf: true });
    return op(str3);
  },
  // a[b] or a[]
  "["([, a, b], out) {
    ...
    if (typeof b[1] === "number") {
      if (b[1] >= 0)
        return f642.load(i322.add(`(i32.trunc_f64_u ${expr3(a)})`, i322.const(b[1] << 3)));
    }
  ...
  "="([, a, b], out) {
    ...
    if (a[0] === "(") {
      ...
      for (let name2 in locals)
        if (!locals[name2].arg && !locals[name2].static)
...
${defer.join(" ")}` : ``) + // defers have 0 stack outcome, so result is still there
    ...
    if (a[0] === "*") {
      ...
      globals[`${func}.${a}`] = { type: "f64", init: "(f64.const nan)" };
