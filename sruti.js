...
var consumeConst = (node, ctx) => {
  ...
  if (cmd === "const")
    return [65 + ["i32", "i64", "f32", "f64"].indexOf(type2), ...encode_exports[type2](node[0])];
  return [
...
function applyUnits(n, unit, ext) {
  if (unit)
    n = expr2(["*", [FLOAT, n], units[unit] || err3(`Unknown unit \`${unit}\``)]);
  if (ext)
  ...
  if (a[0] === ",") {
    ...
    if (b[0] === ",") {
      ...
      if (as.length !== bs.length)
        err3(`Mismatching number of elements in \`${op2}\` operation`);
      return [
        ",",
...
function set(name, init = "") {
  if (!func && name[0] !== "_" || !initing && globals[name])
...
function type(opStr, type2) {
  ...
  if (dif >= opStr.type.length - 1) {
    opStr = float(opStr);
    for (let i = opStr.type.length; i < type2.length; i++)
      opStr += `(${type2[i]}.const nan)`;
...
function pick(count, input) {
  ...
  if (count > input.type.length)
    err3("Picking more members than available");
}
var isConstExpr = (a) => (
...
function compile(node, config = {}) {
  ...
  for (let id2 of ids(lastNode))
    if (!id2.includes(":"))
      exports[id2] = globals[id2] || funcs[id2] || err3("Unknown export member `" + id2 + `'`);
  ...
  "[]"([, [, ...inits]], out) {
    ...
    for (let init of inits) {
      if (init[0] === "..") {
        let [, min, max] = init;
        if (max[1] === Infinity)
          err3(`Arrays cannot be constructed from right-open ranges`);
        if (min[1] === -Infinity && typeof max[1] === "number") {
          if (max[1] < 0)
            err3(`Bad array range`);
...
`), "f64", { buf: true });
  ...
  "="([, a, b], out) {
    ...
    if (a[0] === "(") {
      ...
      if (func)
        err3("Declaring local function `" + name + "`: not allowed");
      if (funcs[name])
        err3("Redefining function `" + name + "`: not allowed");
      ...
      args = args.map((arg) => {
        ...
        if (typeof arg === "string")
          ...
          err3("Unknown function argument");
      ...
      for (let r of returns) {
        if (defers.length)
          r = op(`${type(r, returns.type)}(br $func)`, returns.type);
        else
          r = op(`(return ${type(r, returns.type)})`, returns.type);
        body = body.replace(RETURN, r);
      }
      body = type(op(body, btype), returns.type);
      if (returns.type.length)
        result = `(result ${returns.type.join(" ")})`;
      if (defers.length)
        body = op(`(block $func ${result} ${body})`, body.type);
      let initState;
      defineFn(
        name,
...
${defers.join(" ")}` : ``) + // defers have 0 stack, so result is from body
    ...
    if (typeof a === "string") {
      if (globals[a]?.func)
        err3(`Redefining function '${a}' is not allowed`);
...
` + set(idx2, float(expr3(min))) + set(end, float(expr3(max))) + loop(
  ...
  "-"([, a, b], out) {
    if (!b) {
      ...
      if (res.type.length > 1)
        err3("Group negation: unimplemented");
      if (res.type[0] === "i32")
    ...
    if (aop.type.length > 1 || bop.type.length > 1)
      err3("Group subtraction: unimplemented");
  ...
  "/"([, a, b], out) {
    if (!b) {
      ...
      returns.push(aop2);
      if (!returns.type)
        returns.type = [...aop2.type];
      else
        uptype(returns.type, aop2.type);
  ...
  "++"([, a], out) {
    ...
    if (typeof a !== "string")
      err3("Invalid left hand-side expression in prefix operation");
    let aop = expr3(a);
  ...
  "!"([, a], out) {
    ...
    if (aop.type.length > 1)
      err3("Group inversion: unimplemented");
    if (aop.type[0] === "i32")
  ...
  "?"([, a, b], out) {
    let aop = expr3(a), bop = expr3(b, out);
    if (aop.type.length > 1)
      err3("Group condition is not supported yet.");
    if (!out)
      return op(`(if ${aop.type[0] == "i32" ? aop : `(f64.ne ${aop} (f64.const 0))`} (then ${bop}))`);
  ...
  "~"([, a, b], out) {
    ...
    if (b[0] !== "..")
      err3("Non-range passed as right side of clamp operator");
...
// index.js
var sruti_default = compile;
export {
  compile,
  sruti_default as default,
