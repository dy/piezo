  ...
  Object.assign(expr, {
    ...
    '='([,left,right]) {
      ...
      if (typeof left === 'string') {
        ...
        let {type} = scope[right] || desc(right)
        if (!scope[left]) scope[left] = {type}
        else {
          scope[left].mutable = true
          if (!scope[left].type) scope[left].type = type
          else if (scope[left].type !== type) err('Changing value type from `' + scope[left].type + '` to ' + '`' + type + '`')
        }
        return ['=',left,right]
      }
      // [size]a = ...
      if (left[0] === '[' && left.length > 1) {
        let [,size,name]=left;
        size = expr(size);
        let sizeDesc = scope[size] || desc(size)
    ...
    '->'([,args,body]) {
      ...
      const init = catchArgs(args, scope)
      let out = ['->', expr(init), expr(body[0]==='(' ? body[1] : body)]
      out.scope = scope
      scope = parent
      return out
    },
    ...
    '|'([,a,b]) {
      ...
      if (b[0] === '->') {
        let argScope = {}, [,args,body] = b
        let argInits = catchArgs(args, argScope), [argName, idxName] = Object.keys(argScope)
        ...
        // a | (x,idx) -> body   becomes    (arg = a; i = 0; i < arg[] <| (idx=i; x = arg[i]; body))
        let out = expr(['(',[';',
          ['=','pipe/arg',a],
          ['=','pipe/idx',['int',0]],
          ['<|',
...
// helper for -> and other processors
function catchArgs(args, scope) {
  let init = [';']
  if (args) {
    if (args[0]==='(') [,args]=args;
    ...
    // (a,b)->, (a=1,b=2)->, (a=(1;2))->
    for (let arg of args) {
      if (typeof arg === 'string') scope[arg]={arg:true, type:FLOAT}
      else if (arg[0] === '=') scope[arg[1]]={arg:true, type:FLOAT}, init.push(arg)
      else if (arg[0] === '-<') scope[arg[1]]={arg:true, type:FLOAT}, init.push(arg)
...
export function desc(node) {
  if (!node) return null // null means type will be detected later?
  if (typeof node === 'string') err('Check scope first manually')
  let [op, a, b] = node
  if (op === 'int') return {type:INT}
  if (op === 'flt') return {type:FLOAT}
  if (op === '+' || op === '*' || op === '-') return !b ? desc(a) : (desc(a).type === FLOAT || desc(b).type === FLOAT) ? {type:FLOAT}: {type:INT}
  if (op === '/') return (desc(a).type === INT && desc(b).type === INT) ? {type:INT}: {type:FLOAT}
  if (op === '-<') return desc(a).type === INT && desc(b[1]).type === INT && desc(b[2]).type === INT ? {type:INT} : {type:FLOAT}
