...
var lookup = [];
var token = (op2, prec = SPACE, map, c = op2.charCodeAt(0), l = op2.length, prev2 = lookup[c], word = op2.toUpperCase() !== op2) => lookup[c] = (a, curPrec, from = idx) => curPrec < prec && (l < 2 || cur.substr(idx, l) == op2) && (!word || !parse.id(cur.charCodeAt(idx + l))) && (idx += l, map(a)) || (idx = from, prev2?.(a, curPrec));
...
var nary = (op2, prec) => {
  token(
    op2,
    prec,
    (a, b) => (b = expr(prec), (!a || a[0] !== op2) && (a = [op2, a]), // if beginning of sequence - init node
    b?.[0] === op2 ? [, ...b] = b : b = [b], a.push(...b), // comments can return same-token expr
    a)
  );
};
...
var num = (a) => {
  if (a)
    err();
  let n = next(isNum), sep = "", d = "", unit;
  let c = cur.charCodeAt(idx + 1);
  if (numTypes[cur[idx]] && (isNum(c) || c === PLUS || c === MINUS)) {
    sep = skip();
    if (c === PLUS || c === MINUS)
      d = skip(), d += next(isNum) || err("Bad number " + n + sep + d);
    else
      d = next(isNum);
    if (sep && !d)
      err("Bad number " + n + sep + d);
  }
  if (!n && !d)
    err("Bad number");
  let node = numTypes[sep](n, d);
  if (unit = next((c2) => !isNum(c2) && isId(c2)))
    node.push(unit);
  if (isNum(cur.charCodeAt(idx)))
    node.push(num());
  return node;
};
var numTypes = {
...
function ids(node, set2 = /* @__PURE__ */ new Set()) {
  ...
  if (typeof node === "string")
    ...
    if (node[0] === INT || node[0] === FLOAT)
      return;
    if (node[0] === "=" && node[1][0] === "(")
      return set2.add(node[1][1]), set2;
...
function applyUnits(n, unit, ext) {
  if (unit)
    n = expr2(["*", [FLOAT, n], units[unit] || err2(`Unknown unit \`${unit}\``)]);
  if (ext)
    n = expr2(["+", n, applyUnits(...ext.slice(1))]);
  return n;
}
Object.assign(expr2, {
  [FLOAT]([, n, unit, ext]) {
    if (unit)
      return applyUnits(n, unit, ext);
    return [FLOAT, n];
  },
  [INT]([, n, unit, ext]) {
    if (unit)
  ...
  ";"(node) {
    let list = [];
    for (let i = 1, statement; i < node.length; i++)
      if ((statement = expr2(node[i])) || i === node.length - 1)
        list.push(statement);
  ...
  "[]"([, inits]) {
    ...
    inits = inits.flatMap((el, i) => {
      ...
      if (el[0] === ".." && typeof el[1][1] === "number" && typeof el[2][1] === "number" && Math.abs(el[2][1] - el[1][1]) < 108) {
        ...
        if (from === -Infinity)
          for (let i2 = 0; i2 < to; i2 += step)
            els.push([FLOAT, 0]);
        else if (from < to)
          for (let i2 = from; i2 < to; i2 += step)
            els.push([FLOAT, i2]);
        else
          for (let i2 = from; i2 > to; i2 -= step)
            els.push([FLOAT, i2]);
  ...
  ".."([, a, b]) {
    ...
    if (!a)
      return ["..", [FLOAT, -Infinity], expr2(b)];
    if (!b)
      return ["..", expr2(a), [FLOAT, Infinity]];
    a = expr2(a), b = expr2(b);
    return ["..", a, b];
  },
  "|>"([, a, b]) {
  ...
  if (a[0] === ",") {
    ...
    if (b[0] === ",") {
      ...
      if (as.length !== bs.length)
        err2(`Mismatching number of elements in \`${op2}\` operation`);
      return [
        ",",
...
function f32(input, value, idx2) {
  if (~(idx2 = input.indexOf("nan:"))) {
    value = parseInt(input.slice(idx2 + 4));
    value |= F32_NAN;
    if (input[0] === "-")
      value |= F32_SIGN;
...
function f64(input, value, idx2) {
  if (~(idx2 = input.indexOf("nan:"))) {
    value = BigInt(input.slice(idx2 + 4));
    value |= F64_NAN;
    if (input[0] === "-")
      value |= F64_SIGN;
...
var TYPE = { i32: 127, i64: 126, f32: 125, f64: 124, void: 64, func: 96, funcref: 112 };
...
var parse_default3 = (str3) => {
  ...
  const parseLevel = () => {
    for (let c, root; i < str3.length; ) {
      c = str3.charCodeAt(i);
      if (c === DQUOTE2)
        commit(), buf = str3.slice(i++, i = str3.indexOf('"', i) + 1), commit();
      else if (c === OPAREN) {
        if (str3.charCodeAt(i + 1) === SEMIC)
          i = str3.indexOf(";)", i) + 2;
        else
          commit(), i++, (root = level).push(level = []), parseLevel(), level = root;
      } else if (c === SEMIC)
        i = str3.indexOf("\n", i) + 1;
      else if (c <= SPACE3)
        commit(), i++;
...
var compile_default = (nodes) => {
  ...
  if (typeof nodes[0] === "string" && nodes[0] !== "module")
    nodes = [nodes];
  ...
  for (let cb of postcall)
    cb?.();
  for (let name in sections) {
    let items = sections[name];
    if (items.importc)
      items = items.slice(items.importc);
    if (!items.length)
      continue;
...
var build = {
  ...
  type([, typeName, [kind, ...sig]], ctx) {
    if (kind !== "func")
      err3(`Unknown type kind '${kind}'`);
    const [idx2] = consumeType(sig, ctx);
    if (typeName)
      ctx.type[typeName] = idx2;
  },
  // (func $name? ...params result ...body)
  func([, ...body], ctx) {
    let locals2 = [], blocks = [];
    if (body[0]?.[0] === "$")
      ctx.func[body.shift()] = ctx.func.length;
    if (body[0]?.[0] === "export")
      build.export([...body.shift(), ["func", ctx.func.length]], ctx);
    let [typeIdx, params, result] = consumeType(body, ctx);
    ctx.func.push([typeIdx]);
    while (body[0]?.[0] === "local") {
      let [, ...types] = body.shift(), name;
      if (types[0][0] === "$")
        params[name = types.shift()] ? err3("Ambiguous name " + name) : locals2[name] = params.length + locals2.length;
      locals2.push(...types.map((t) => TYPE[t]));
    }
    let locTypes = locals2.reduce((a, type) => (type == a[a.length - 1] ? a[a.length - 2]++ : a.push(1, type), a), []);
    const consume = (nodes, out = []) => {
      ...
      if (group2 = Array.isArray(op2)) {
        ...
        opCode = OP.indexOf(op2);
      if (opCode >= 252) {
        immed = [252, opCode %= 252];
        if (!(opCode & 2))
        ...
        let o = { align: ALIGN[op2], offset: 0 }, param;
        while (args[0]?.includes("="))
          param = args.shift().split("="), o[param[0]] = Number(param[1]);
        ...
        immed = uleb(id2 = fnName[0] === "$" ? ctx.func[fnName] ?? err3("Unknown function `" + fnName + "`") : fnName);
      } else if (opCode == 17) {
        ...
        blocks.push(opCode);
        if (opCode < 4 && args[0]?.[0] === "$")
          blocks[args.shift()] = blocks.length;
        ...
        while (!Array.isArray(args[0]))
          id2 = args.shift(), immed.push(...uleb(id2[0][0] === "$" ? blocks.length - blocks[id2] : id2));
        immed.unshift(...uleb(immed.length - 1));
      } else if (opCode < 0)
        err3(`Unknown instruction \`${op2}\``);
      if (group2) {
        while (args.length)
          consume(args, out);
      }
  ...
  memory([, ...parts], ctx) {
    if (parts[0][0] === "$")
      ctx.memory[parts.shift()] = ctx.memory.length;
    if (parts[0][0] === "export")
      build.export([...parts.shift(), ["memory", ctx.memory.length]], ctx);
  ...
  global([, ...args], ctx) {
    ...
    if (name)
      ctx.global[name] = ctx.global.length;
  ...
  table([, ...args], ctx) {
    let name = args[0][0] === "$" && args.shift();
    if (name)
      ctx.table[name] = ctx.table.length;
    let lims = range(args);
  ...
  import([, mod, field, ref], ctx) {
    ...
    if (kind === "func") {
      ...
      throw Error("Unimplemented " + kind);
    ctx.import.push([...str2(mod), ...str2(field), KIND[kind], ...details]);
  },
  // (data (i32.const 0) "\aa" "\bb"?)
  data([, offset, ...inits], ctx) {
    ctx.data.push([0, ...iinit(offset, ctx), ...str2(inits.map((i) => i[0] === '"' ? i.slice(1, -1) : i).join(""))]);
  },
  // (start $main)
  start([, name], ctx) {
    if (!ctx.start.length)
      ctx.start.push([name[0] === "$" ? ctx.func[name] : name]);
  }
};
var iinit = ([op2, literal], ctx) => op2[0] === "f" ? [op2[1] === "3" ? OP_F32_CONST : OP_F64_CONST, ...(op2[1] === "3" ? f32 : f64)(literal), OP_END] : [op2[1] === "3" ? OP_I32_CONST : OP_I64_CONST, ...(op2[1] === "3" ? leb : bigleb)(literal[0] === "$" ? ctx.global[literal] : literal), OP_END];
...
function call(name, ...args) {
  if (!funcs[name])
    err2(`Unknown function call '${name}'`);
  return op(`(call $${name} ${args.join(" ")})`, funcs[name].type);
}
function float(opStr) {
  if (opStr.type[0] === "f64")
    return opStr;
  if (opStr.startsWith("(i32.const"))
    return op(opStr.replace("(i32.const", "(f64.const"), "f64");
...
function include(name) {
  if (!stdlib_default[name])
    err2("Unknown include `" + name + "`");
  let code = stdlib_default[name];
  let type = code.match(/\(result\s+([^\)]+)\)/i)?.[1].trim().split(/\s+/);
  if (!funcs[name])
    fun(name, code, type);
}
function fun(name, code, type) {
...
function pick(count, input) {
  ...
  if (input.type.length === count)
    return input;
  if (count < input.type.length)
    return op(input + `(drop)`.repeat(input.type.length - count), input.type.slice(0, count));
  if (count > input.type.length)
    err2("Picking more members than available");
}
...
function compile(node, config = {}) {
  ...
  for (let id2 of ids(lastNode))
    if (!id2.includes(":"))
      exports[id2] = globals[id2] || err2("Unknown export member `" + id2 + `'`);
...
`;
  ...
  for (let name in globals)
    if (!globals[name].import)
      code += `(global $${name} (mut ${globals[name].type}) (${globals[name].type}.const ${globals[name].init || 0}))
`;
  code += `
`;
...
function expr3(statement, out = true) {
  ...
  if (statement[0] in expr3)
    return statement.expr = expr3[statement[0]](statement, out) || op();
  err2(`Unknown operation ${statement}`);
}
Object.assign(expr3, {
  ...
  ";"([, ...statements], out) {
    if (!statements[statements.length - 1])
      out = false;
    let list = statements.map((s, i) => i === statements.length - 1 ? expr3(s, out) : expr3(s, false));
    if (!list.length)
      return;
  ...
  // a,b,c. - voids result
  "."([, a]) {
    return expr3(a, false);
  },
  ","([, ...statements], out) {
    let list = [];
    for (let s of statements)
      list.push(expr3(s, out));
  ...
  "("([, name, [, ...args]], out) {
    ...
    if (state) {
      ...
      ;
      (globals[func].substate ||= {})[name] = callerState.length;
      callerState.length += state.length;
    }
    if (out)
      return call(name, args.map((arg) => float(expr3(arg))));
  ...
  "[]"([, [, ...inits]], out) {
    mem ||= 0;
    if (!inits.length)
      return out && op(f642.const(0), "f64");
    depth++;
    if (function isStatic(inits2) {
      return inits2.every((x) => typeof x[1] === "number" || (x[0] === "[" && isStatic(x.slice(1)), false));
    }(inits)) {
      ...
      if (func) {
        ...
        return include("malloc"), include("arr.ref"), op(
          set(tmp, call("malloc", i32.const(f64s.length << 3))) + `(memory.copy ${get(tmp)} ${i32.const(offset)} ${i32.const(f64s.length << 3)})` + call("arr.ref", get(tmp), i32.const(f64s.length)),
          "f64"
        );
      } else {
    ...
    locals[ptr] = { type: "i32" };
    include("malloc");
    let str3 = set(ptr, tee(start, call("malloc", i32.const(HEAP_SIZE))));
    for (let init of inits) {
      if (init[0] === "..") {
        let [, min, max] = init;
        if (max[1] === Infinity)
          err2(`Arrays cannot be constructed from right-open ranges`);
        if (min[1] === -Infinity && typeof max[1] === "number") {
          if (max[1] < 0)
            err2(`Bad array range`);
          str3 += set(ptr, i32.add(get(ptr), i32.const(max[1] << 3)));
        } else {
          ...
          str3 += loop(
            cond(
              ...
              f642.store(get(ptr), get(i)) + set(ptr, i32.add(get(ptr), i32.const(8))) + set(i, f642.add(get(i), f642.const(1))) + `(br 1)`
            )
          );
        }
      } else if (init[0] === "|>") {
        ...
        str3 += f642.store(i32.sub(tee(ptr, i32.add(get(ptr), i32.const(8))), i32.const(8)), float(expr3(init)));
    }
    include("malloc"), include("arr.ref");
    str3 += `(global.set $__mem ${get(ptr)})`;
    depth--;
    if (out)
      return op(str3 + call("arr.ref", `${get(start)} (i32.shr_u (i32.sub ${get(ptr)} ${get(start)}) ${i32.const(3)})
`), "f64", { buf: true });
    return op(str3);
  },
  // a[b] or a[]
  "["([, a, b], out) {
    ...
    if (!out)
      return op(expr3(a, false) + expr3(b, false));
    if (!b)
      return call("arr.len", expr3(a));
    if (typeof b[1] === "number") {
      if (b[1] >= 0)
        return f642.load(i32.add(`(i32.trunc_f64_u ${expr3(a)})`, i32.const(b[1] << 3)));
    }
  ...
  "="([, a, b], out) {
    ...
    if (a[0] === "(") {
      ...
      if (func)
        err2("Declaring local function `" + name + "`: not allowed");
      if (globals[name])
        err2("Redefining function `" + name + "`: not allowed");
      ...
      args = args.map((arg) => {
        ...
        if (typeof arg === "string")
          ...
          [, name2, init] = arg, inits.push(["?", ["!=", name2, name2], ["=", name2, init]]);
        else if (arg[0] === "~")
          [, name2, init] = arg, inits.push(["~=", name2, arg[2]]);
        else
          err2("Unknown function argument");
        locals[name2] = { arg: true, type: "f64" };
        dfn.push(`(param $${name2} f64)`);
        return name2;
      });
      globals[name] = { func: true, args, type: "i32" };
      body.splice(1, 0, ...inits);
      returns = [];
      result = expr3(body);
      if (returns.length)
        result = float(result);
      ...
      for (let name2 in locals)
        if (!locals[name2].arg)
          dfn.push(`(local $${name2} ${locals[name2].type})`);
      globals[name].state?.forEach((name2) => {
        defer.push(f642.store(get(name2 + ".adr"), get(name2)));
      });
      locals = rootLocals;
      let initState;
...
` + set(idx2, float(expr3(min))) + set(end, float(expr3(max))) + loop(
      ...
      console.log(a, b);
      err2("loop over list: unimplemented");
    }
    depth--;
  ...
  "-"([, a, b], out) {
    if (!b) {
      ...
      if (res.type.length > 1)
        err2("Group negation: unimplemented");
      if (res.type[0] === "i32")
        return op(`(i32.sub (i32.const 0) ${res})`, "i32");
    ...
    if (aop.type.length > 1 || bop.type.length > 1)
      err2("Group subtraction: unimplemented");
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return op(`(i32.sub ${aop} ${bop})`, "i32");
  ...
  "*"([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type.length > 1 || bop.type.length > 1) {
      err2("Complex group multiplication is not supported");
    }
  ...
  "**"([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (b[1] === 0.5)
      return op(`(f64.sqrt ${float(aop)})`, "f64");
  ...
  "%%"([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return include("i32.modwrap"), call("i32.modwrap", aop, bop);
    return include("f64.modwrap"), include("f64.rem"), call("f64.modwrap", float(aop), float(bop));
  },
  "++"([, a], out) {
    if (!out)
      return expr3(["=", a, ["+", a, [INT, 1]]], false);
    if (a[0] === "[")
      return expr3(["-", ["=", a, ["+", a, [INT, 1]]], [INT, 1]]);
    if (typeof a !== "string")
      err2("Invalid left hand-side expression in prefix operation");
    let aop = expr3(a);
  ...
  "!"([, a], out) {
    ...
    if (aop.type.length > 1)
      err2("Group inversion: unimplemented");
    if (aop.type[0] === "i32")
      return op(`(if (result i32) (i32.eqz ${aop}) (then (i32.const 1)) (else (i32.const 0)))`, "i32");
  ...
  "<"([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return op(`(i32.lt_s ${aop} ${bop})`, "i32");
  ...
  "<="([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return op(`(i32.le_s ${aop} ${bop})`, "i32");
  ...
  ">"([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return op(`(i32.gt_s ${aop} ${bop})`, "i32");
  ...
  ">="([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return op(`(i32.ge_s ${aop} ${bop})`, "i32");
  ...
  "=="([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return op(`(i32.eq_s ${aop} ${bop})`, "i32");
  ...
  "!="([, a, b], out) {
    ...
    if (!out)
      return op(aop + bop);
    if (aop.type[0] === "i32" && bop.type[0] === "i32")
      return op(`(i32.ne_s ${aop} ${bop})`, "i32");
  ...
  "||"([, a, b], out) {
    ...
    if (!out)
      return op(`(if ${aop.type[0] == "i32" ? aop : `(f64.ne ${aop} (f64.const 0))`} (else ${bop}))`);
    if (aop.type[0] == "f64")
      return op(`${pick(2, aop)}(if (param f64) (result f64) (f64.ne (f64.const 0)) (then) (else (drop) ${float(bop)}))`, "f64");
    if (bop.type[0] == "i32")
  ...
  "&&"([, a, b], out) {
    let aop = expr3(a), bop = expr3(b, out);
    if (!out)
      return op(`(if ${aop.type[0] == "i32" ? aop : `(f64.ne ${aop} (f64.const 0))`} (then ${bop}))`);
    if (aop.type[0] == "f64")
      return op(`${pick(2, aop)}(if (param f64) (result f64) (f64.ne (f64.const 0)) (then (drop) ${float(bop)}))`, "f64");
    if (bop.type[0] == "i32")
  ...
  "?"([, a, b], out) {
    let aop = expr3(a), bop = expr3(b, out);
    if (aop.type.length > 1)
      err2("Group condition is not supported yet.");
    if (!out)
      return op(`(if ${aop.type[0] == "i32" ? aop : `(f64.ne ${aop} (f64.const 0))`} (then ${bop}))`);
  ...
  "?:"([, a, b, c], out) {
    ...
    if (aop.type.length > 1)
      err2("Group condition is not supported yet.");
    if (bop.type[0] === "i32" && cop.type[0] === "i32")
      return op(`(if ${out ? `(result i32)` : ``} ${aop.type[0] == "i32" ? aop : `(f64.ne ${aop} (f64.const 0))`} (then ${bop} ) (else ${cop}))`, out ? "i32" : []);
  ...
  "~"([, a, b], out) {
    ...
    if (b[0] !== "..")
      err2("Non-range passed as right side of clamp operator");
    let [, min, max] = b, aop = expr3(a), minop = min && expr3(min, out), maxop = max && expr3(max, out);
    if (!out)
      return op(minop + maxop);
    if (!max) {
      if (aop.type[0] === "i32" && minop.type[0] === "i32")
        return include("i32.smax"), call("i32.max", aop, minop);
      return op(`(f64.max ${float(aop)} ${float(minop)})`, "f64");
    }
    if (!min) {
      if (aop.type[0] === "i32" && maxop.type[0] === "i32")
        return include("i32.smin"), call("i32.min", aop, maxop);
...
// index.js
var mel_default = compile;
export {
  compile,
  mel_default as default,
